(function () {
  ...
  if (gl) {
    try {
      ...
      gl.attachShader(program, compile(gl.VERTEX_SHADER, 'precision highp float;attribute vec3 a_position;attribute vec3 a_color;attribute vec2 a_light;uniform vec2 u_resolution;uniform vec2 u_pointer;uniform float u_scale;uniform float u_dpr;uniform float u_time;varying vec3 v_color;varying float v_alpha;void main(){float rot=u_time*.0045;float breathe=1.+sin(u_time*.15)*.006;vec2 p=mat2(cos(rot),-sin(rot),sin(rot),cos(rot))*a_position.xy*breathe;float inclinedY=p.y*.63+a_position.z*.78;vec2 projected=mat2(.984,.178,-.178,.984)*vec2(p.x,inclinedY);projected+=u_pointer*a_position.z*.075;float radius=min(u_resolution.x*.48,u_resolution.y*.60)*u_scale;vec2 pixel=vec2(u_resolution.x*.54,u_resolution.y*.49)+projected*radius;gl_Position=vec4(pixel/u_resolution*2.-1.,0.,1.);gl_PointSize=max(1.,a_light.x*u_dpr*pow(u_scale,.35));v_color=a_color;float shimmer=.96+.04*sin(u_time*.22+a_position.x*53.+a_position.y*41.);v_alpha=a_light.y*shimmer/pow(u_scale,.2);}'));
